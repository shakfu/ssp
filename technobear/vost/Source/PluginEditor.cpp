...
PluginEditor::PluginEditor(PluginProcessor& p) : base_type(&p, MAX_VIEW), processor_(p) {
    ...
    addParamPage(std::make_shared<pcontrol_type>(processor_.params_.select, 1.0f, 0.01), nullptr, nullptr, nullptr,
                 view, Colours::orange);

    addButtonPage(std::make_shared<bcontrol_type>(processor_.params_.slew, 24, Colours::lightskyblue),
                  std::make_shared<bcontrol_type>(processor_.params_.morph, 24, Colours::orange), nullptr, nullptr,
                  nullptr, nullptr, nullptr, nullptr, view);
    view++;
    ...
    for (unsigned layer = 0; layer < MAX_LAYERS; layer++) {
        ...
        for (unsigned vPage = 0; vPage < MAX_SIG / 4; vPage++) {
            int sigp = vPage * 4;
            addParamPage(std::make_shared<pcontrol_type>(l.volts_[sigp + 0]->val, 0.25),
                         std::make_shared<pcontrol_type>(l.volts_[sigp + 1]->val, 0.25),
                         std::make_shared<pcontrol_type>(l.volts_[sigp + 2]->val, 0.25),
                         std::make_shared<pcontrol_type>(l.volts_[sigp + 3]->val, 0.25), view, clrs[view % L_CLRS]);
