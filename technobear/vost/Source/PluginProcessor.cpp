...
PluginProcessor::PluginProcessor(const AudioProcessor::BusesProperties& ioLayouts,
    ...
    init();
    float select = 0.0f;
    for (int i = 0; i < MAX_SIG_OUT; i++) { lastVolt_[i] = getCurrentVolt(select, i, params_.morph.getValue() > 0.5f); }
}
...
String getVoltPid(unsigned lid, unsigned vid) {
    String pid = getLayerPid(lid) + String(ID::separator) + String(ID::volts) + String(ID::separator) + String(vid) +
...
PluginProcessor::VoltParam::VoltParam(AudioProcessorValueTreeState& apvt, unsigned lid, unsigned id)
    : val(*apvt.getParameter(getVoltPid(lid, id))), id_(id), lid_(lid), pid_(getVoltPid(lid, id)) {
}


PluginProcessor::Layer::Layer(AudioProcessorValueTreeState&, unsigned id) : id_(id), pid_(getLayerPid(id)) {
}

PluginProcessor::PluginParams::PluginParams(AudioProcessorValueTreeState& apvt)
    : slew(*apvt.getParameter(ID::slew)), select(*apvt.getParameter(ID::select)), morph(*apvt.getParameter(ID::morph)) {
...
const String PluginProcessor::getInputBusName(int channelIndex) {
    static String inBusName[I_MAX] = { "Select", "In A", "In B", "In C", "In D", "In E", "In F", "In G", "In H",
...
void PluginProcessor::processBlock(AudioSampleBuffer& buffer, MidiBuffer& midiMessages) {
    ...
    float select = std::min(std::max(((params_.select.getValue() + selectOffset) * float(MAX_LAYERS - 1)), 0.0f),
                            float(MAX_LAYERS - 1));
