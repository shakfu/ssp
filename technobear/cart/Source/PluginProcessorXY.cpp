...
unsigned PluginProcessor::setLayerStep(Steps& steps, unsigned seqstep, LayerData& ld) {
    ...
    if (!ld.p_.fun_op_sleep_) {
        ...
        for (unsigned i = 0; i < MAX_STEPS; i++) {
            ...
            if (steps[pos]->access.getValue() > 0.5f) { return step; }
            if (!ld.reverse_) {
...
void PluginProcessor::processLayer(float clkIn, float modIn, float cvIn, Steps& steps, LayerData& ld, float& o_cv,
                                   bool& o_gate) {
    ...
    bool modClkTrig =
        ld.p_.fun_mod_mode_ == MOD_MODE_CLK && (modIn >= trigLevel && ld.lastModIn_ < trigLevel) && clkIn < clockLevel;
    ...
    ld.pos_ = snakes_.getPosition((ld.p_.snake_ + ld.snakeOffset_) % snakes_.size(),
    ...
    if (access) { ld.targetCv_ = (activeStep.cv.getValue() * 2.0f) - 1.0f; }

    if (ld.p_.fun_cv_mode_ == CV_MODE_ADD) {
        ld.cvOffset_ = cvIn;
    } else if (ld.p_.fun_cv_mode_ == CV_MODE_SH) {
        if (modIn >= trigLevel && (modIn >= trigLevel && ld.lastModIn_ < trigLevel)) { ld.cvOffset_ = cvIn; }
    }

    if (ld.p_.scale_ > 0.0f)
        ld.targetCv_ = quantizeCv(ld.p_.scale_, ld.p_.root_, ld.cvOffset_ + ld.targetCv_);
    else
        ld.targetCv_ += ld.cvOffset_;
    ...
    if (gate) {
        if (ld.clkTrig_) {
            if (modClkTrig || ld.p_.fun_op_trig_) { ld.gateTime_ = trigGateTime; }
        }
