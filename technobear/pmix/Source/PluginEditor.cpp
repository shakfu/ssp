...
PluginEditor::PluginEditor(PluginProcessor& p) : base_type(&p, 3), processor_(p) {
    ...
    for (unsigned v = 0; v < 3; v++) {
        ...
        addParamPage(std::make_shared<pcontrol_type>(td0.level[0], cc, fc),
                     std::make_shared<pcontrol_type>(td1.level[0], cc, fc),
                     std::make_shared<pcontrol_type>(td2.level[0], cc, fc),
                     std::make_shared<pcontrol_type>(td3.level[0], cc, fc), v, pc);
        addParamPage(std::make_shared<pcontrol_type>(td0.pan, cc, fc), std::make_shared<pcontrol_type>(td1.pan, cc, fc),
                     std::make_shared<pcontrol_type>(td2.pan, cc, fc), std::make_shared<pcontrol_type>(td3.pan, cc, fc),
                     v, pc);
        addParamPage(std::make_shared<pcontrol_type>(td0.level[1], cc, fc),
                     std::make_shared<pcontrol_type>(td1.level[1], cc, fc),
                     std::make_shared<pcontrol_type>(td2.level[1], cc, fc),
                     std::make_shared<pcontrol_type>(td3.level[1], cc, fc), v, pc);
        addParamPage(std::make_shared<pcontrol_type>(td0.level[2], cc, fc),
                     std::make_shared<pcontrol_type>(td1.level[2], cc, fc),
                     std::make_shared<pcontrol_type>(td2.level[2], cc, fc),
                     std::make_shared<pcontrol_type>(td3.level[2], cc, fc), v, pc);
        addParamPage(std::make_shared<pcontrol_type>(td0.level[3], cc, fc),
                     std::make_shared<pcontrol_type>(td1.level[3], cc, fc),
                     std::make_shared<pcontrol_type>(td2.level[3], cc, fc),
                     std::make_shared<pcontrol_type>(td3.level[3], cc, fc), v, pc);
        addParamPage(std::make_shared<pcontrol_type>(td0.gain, cc, fc),
                     std::make_shared<pcontrol_type>(td1.gain, cc, fc),
                     std::make_shared<pcontrol_type>(td2.gain, cc, fc),
                     std::make_shared<pcontrol_type>(td3.gain, cc, fc), v, pc);

        unsigned fh = 24;
        addButtonPage(std::make_shared<bcontrol_type>(td0.mute, fh, Colours::red),
                      ...
                      std::make_shared<bcontrol_type>(td3.solo, fh, Colours::yellow), v);
        addButtonPage(std::make_shared<bcontrol_type>(td0.cue, fh, Colours::lightskyblue),
                      ...
                      std::make_shared<bcontrol_type>(td3.ac, fh, Colours::orange), v);
    }
...
void PluginEditor::drawView(Graphics& g) {
    ...
    for (unsigned i = 0; i < PluginProcessor::IN_T_MAX; i++) { inTracks_[i].enabled(processor_.isInputEnabled(i)); }
...
void PluginEditor::onLeftShiftButton(bool v) {
    ...
    if (v) {
        ...
        if (viewHeldCount_ == 0) { setView(unheldView_); }
        viewHeldCount_ = 0;
        unheldView_ = view_;
    }
}
