...
const String PluginProcessor::getInputBusName(int channelIndex) {
    if (channelIndex >= I_LAYER1_RATE && channelIndex <= I_LAYER4_END) {
        ...
        switch (sig) {
            case 0: return lstr + std::string(" Rate");
            case 1: return lstr + std::string(" Begin");
            case 2: return lstr + std::string(" End");
    ...
    switch (channelIndex) {
        ...
        default: {
            if (channelIndex < O_MAX) {
                ...
                std::string lstr = std::string("Layer ") + std::to_string(layer + 1);
                switch (sig) {
                    case 0: return lstr + std::string(" Out");
                    default: return lstr + std::string(" ZZ ") + std::to_string(sig);
                }
            }
        }
