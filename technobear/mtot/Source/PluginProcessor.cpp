...
PluginProcessor::PluginProcessor(const AudioProcessor::BusesProperties& ioLayouts,
    ...
    init();
    for (int i = 0; i < I_MAX; i++) { lastMidi_[i] = 0; }
    pitchbend_ = 8192;
}
...
const String PluginProcessor::getInputBusName(int channelIndex) {
    switch (channelIndex) {
        case I_CV_A: return "CV A";
        case I_CV_B: return "CV B";
        case I_CV_C: return "CV C";
        case I_CV_D: return "CV D";
        case I_CV_E: return "CV E";
        case I_CV_F: return "CV F";
        case I_CV_G: return "CV G";
        case I_CV_H: return "CV H";
        case I_VOCT: return "VOct";
        case I_GATE: return "Gate";
        case I_VEL: return "Vel";
        default:;
    }
...
int PluginProcessor::getCCNum(int idx) {
    switch (idx) {
        case I_CV_A: return GET_P_VAL(params_.cv_a);
        case I_CV_B: return GET_P_VAL(params_.cv_b);
        case I_CV_C: return GET_P_VAL(params_.cv_c);
        case I_CV_D: return GET_P_VAL(params_.cv_d);
        case I_CV_E: return GET_P_VAL(params_.cv_e);
        case I_CV_F: return GET_P_VAL(params_.cv_f);
        case I_CV_G: return GET_P_VAL(params_.cv_g);
        case I_CV_H: return GET_P_VAL(params_.cv_h);
        default:;
    }
    return 0;
}
...
void PluginProcessor::processBlock(AudioSampleBuffer& buffer, MidiBuffer& midiMessages) {
    ...
    if (!midimsgs.isEmpty()) { sendMidiMessagesNow(midimsgs); }
}
