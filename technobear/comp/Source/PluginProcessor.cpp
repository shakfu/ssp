...
AudioProcessorValueTreeState::ParameterLayout PluginProcessor::createParameterLayout() {
    ...
    params.add(
        std::make_unique<ssp::BaseFloatParameter>(ID::threshold, "Threshold", -80.0f, 0.0f, -12.0f));    // 0.0 -> -80.
...
const String PluginProcessor::getInputBusName(int channelIndex) {
    static String inBusName[I_MAX] = { "In L", "In R", "Sidechain" };
...
void PluginProcessor::processBlock(AudioSampleBuffer& buffer, MidiBuffer& midiMessages) {
    ...
    if (stereoOut && !isInputEnabled(I_RIGHT)) {
        for (int s = 0; s < n; s++) { buffer.setSample(O_RIGHT, s, buffer.getSample(O_LEFT, s)); }
    }
