...
PluginProcessor::PluginProcessor(const AudioProcessor::BusesProperties& ioLayouts,
    ...
    for (int i = 0; i < rnbo_.nInputs_; i++) { rnbo_.inputBuffers_[i] = nullptr; }
    for (int i = 0; i < rnbo_.nOutputs_; i++) { rnbo_.outputBuffers_[i] = nullptr; }
    ...
    for (int i = 0; i < rnbo_.nParams_; i++) {
        ...
        if (info.visible) { nameToRnboIdMap_.insert(std::make_pair(pid, i)); }
    }
    rnbo_.lastParamVals_ = new float[rnbo_.nParams_];
    for (int i = 0; i < rnbo_.nParams_; i++) { rnbo_.lastParamVals_[i] = -100.0; }
    ...
    for (auto& param : nameToRnboIdMap_) {
        ...
        if (info.enumValues) {
            ...
            if (info.steps < 2) {
                ...
                params.add(
                    std::make_unique<ssp::BaseFloatParameter>(id, desc, info.min, info.max, info.initialValue, inc));
            }
        }
    }
...
const String PluginProcessor::getInputBusName(int channelIndex) {
    if (channelIndex >= I_LAYER1_TRIG && channelIndex <= I_LAYER4_PAN) {
        ...
        switch (sig) {
            case 0: return lstr + std::string(" Trig");
            case 1: return lstr + std::string(" Start");
            case 2: return lstr + std::string(" Len");
            case 3: return lstr + std::string(" Rate");
            case 4: return lstr + std::string(" Pan");
            default: return lstr + std::string("ZZ ") + std::to_string(sig);
        }
    }
...
const String PluginProcessor::getOutputBusName(int channelIndex) {
    switch (channelIndex) {
        case O_SUM_L: return String("Sum L");
        case O_SUM_R: return String("Sum R");
        default: {
            if (channelIndex < O_MAX) {
                ...
                std::string lstr = std::string("Layer ") + std::to_string(layer + 1);
                switch (sig) {
                    case 0: return lstr + std::string(" L");
                    case 1: return lstr + std::string(" R");
                    default: return lstr + std::string(" ZZ ") + std::to_string(sig);
                }
            }
        }
...
void PluginProcessor::fillLayerData(unsigned lidx, float* data, unsigned sz, float& cur, float& start, float& len,
    ...
    for (int i = 0; i < layer.size_ && i < sz; i++) { data[i] = layer.data_[i * dinc]; }
...
void PluginProcessor::allocateLayerData(unsigned lidx, unsigned newSize) {
    ...
    if (layer.size_ != newSize) {
        ...
        if (layer.data_ && layer.size_ > 0) {
            for (int x = 0; x < newSize; x++) { newData[x] = x < layer.size_ ? layer.data_[x] : 0.0f; }
        } else {
            for (int x = 0; x < newSize; x++) { newData[x] = 0.0f; }
        }
...
void PluginProcessor::setExternalData(unsigned lidx, float* newData, float newSize) {
    ...
    rnbo_.patch_.setExternalData(id.c_str(), (char*)layer.data_, layer.size_ * sizeof(float) / sizeof(char), type,
                                 &freeLayer);
}
...
void PluginProcessor::processBlock(AudioSampleBuffer& buffer, MidiBuffer& midiMessages) {
    ...
    rnbo_.patch_.process(rnbo_.inputBuffers_, rnbo_.nInputs_, rnbo_.outputBuffers_, rnbo_.nOutputs_, rnbo_.rnboIOSize_);
...
bool PluginProcessor::loadLayerFile(unsigned lidx, const std::string& fn, bool wait) {
    ...
    if (file.exists() && !file.isDirectory()) {
        ...
        if (reader.get() != nullptr) {
            ...
            if (reader->read(&fileBuffer, 0, fileSampleSize, 0, true, true)) {
                ...
                for (int i = 0; i < loadData.size_; i++) { loadData.data_[i] = fileBuffer.getSample(0, i); }
...
bool PluginProcessor::saveLayerFile(unsigned lidx, const std::string& fn) {
    ...
    for (int i = 0; i < layer.size_; i++) { fileBuffer.setSample(0, i, layer.data_[i]); }
    ...
    writer.reset(
        format.createWriterFor(new FileOutputStream(wavFile), sampleRate_, fileBuffer.getNumChannels(), 24, {}, 0));
    if (writer != nullptr) {
