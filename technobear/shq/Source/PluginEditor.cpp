...
PluginEditor::PluginEditor(PluginProcessor& p) : base_type(&p), processor_(p) {
    addParamPage(std::make_shared<pcontrol_type>(processor_.params_.root, 1.0f, 1.0f),
                 std::make_shared<pcontrol_type>(processor_.params_.scale, 1.0f, 1.0f), nullptr, nullptr);

    addButtonPage(std::make_shared<bcontrol_type>(processor_.params_.quant, 24, Colours::lightskyblue), nullptr,
