...
public:
    ...
    std::shared_ptr<Algo> algo(unsigned e) {
        if (e < MAX_ENG)
            return algo_[e];
        else
            return nullptr;
    ...
    void nextAlgo(unsigned e) {
        if (e < MAX_ENG) {
            ...
            if (it == algoDisplayOrder_.end() || it == algoDisplayOrder_.end() - 1)
                it = algoDisplayOrder_.begin();
            else
                it++;
            t = *it;
    ...
    void prevAlgo(unsigned e) {
        if (e < MAX_ENG) {
            unsigned t = algo_[e]->type();
            std::vector<unsigned>::iterator it = std::find(algoDisplayOrder_.begin(), algoDisplayOrder_.end(), t);
            if (it == algoDisplayOrder_.end() || it == algoDisplayOrder_.begin())
                it = algoDisplayOrder_.end() - 1;
            else
                it--;
            t = *it;
            // Logger::writeToLog("algo# " + String(t));
