...
public:
    AgFloatParam(const std::string& n, const std::string& d, float init = 0.0f, float min = 0.0f, float max = 1.0f,
                 float step = 0.01f)
...
public:
    AgIntParam(const std::string& n, const std::string& d, int init = 0, int min = 0, int max = 127, int step = 1)
        : AlgoParam(n, d), val_(init), init_(init), min_(min), max_(max), step_(step) {
        ;
    }
    void inc() override;
...
public:
    ...
    // audio thread
    virtual void process(const float* x, const float* y, const float* z, float* a, float* b, unsigned n) = 0;
