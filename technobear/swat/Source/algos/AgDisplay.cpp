...
void AgDisplay::process(const float* x, const float* y, const float* z, float* a, float* b, unsigned ns) {
    int n = ns;
    ...
    if (a != nullptr) { FloatVectorOperations::fill(a, 0.0f, n); }


    if (b != nullptr) { FloatVectorOperations::fill(b, 0.0f, n); }
}
...
void AgDisplay::paint(Graphics& g) {
    ...
    g.drawSingleLineText("X : " + String::formatted("%5.3f", dx) + "  -  " + String::formatted("%4.2f", dx * 5.0f) +
                             "V - " + getNoteValue(dx),
                         x, y);
    y += space * 2;
    g.drawSingleLineText("Y : " + String::formatted("%5.3f", dy) + "  -  " + String::formatted("%4.2f", dy * 5.0f) +
                             "V - " + getNoteValue(dy),
                         x, y);
    y += space * 2;
    g.drawSingleLineText("Z : " + String::formatted("%5.3f", dz) + "  -  " + String::formatted("%4.2f", dz * 5.0f) +
                             "V - " + getNoteValue(dz),
                         x, y);
}
