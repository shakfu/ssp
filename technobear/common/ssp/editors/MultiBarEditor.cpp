...
MultiBarEditor::MultiBarEditor(BaseProcessor* p, unsigned maxView) : base_type(p, maxView) {
}

MultiViewEditor::ControlPage MultiBarEditor::addParamPage(std::shared_ptr<BaseParamControl> c1,
                                                          std::shared_ptr<BaseParamControl> c2,
                                                          std::shared_ptr<BaseParamControl> c3,
                                                          std::shared_ptr<BaseParamControl> c4, unsigned view,
                                                          juce::Colour clr) {
...
void MultiBarEditor::setView(unsigned v) {
    ...
    if (lastV != nextV) {
        ...
        {  // enable new buttons/controls
            ...
            unsigned pageN = 0;
            if (paramPage_ >= view.controlPages_.size()) { paramPage_ = view.controlPages_.size() - 1; }
            for (auto ppage : view.controlPages_) {
                for (auto i = 0; i < 4; i++) {
                    auto& c = ppage.control_[i];
                    if (c != nullptr) {
