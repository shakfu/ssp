...
PluginEditor::PluginEditor(PluginProcessor& p) : base_type(&p, MAX_VIEW), processor_(p) {
    ...
    unsigned f = 0;
    addParamPage(std::make_shared<pcontrol_type>(processor_.getFilter(f).cutoff, fltCoarse, fltFin),
                 std::make_shared<pcontrol_type>(processor_.getFilter(f).res, 0.1, 0.01f), nullptr, nullptr, view,
                 clrs[f % L_CLRS]);
