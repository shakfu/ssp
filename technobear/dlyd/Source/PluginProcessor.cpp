...
AudioProcessorValueTreeState::ParameterLayout PluginProcessor::createParameterLayout() {
    ...
    for (unsigned tid = 0; tid < MAX_TAPS; tid++) {
        ...
        tap->addChild(std::make_unique<ssp::BaseFloatParameter>(getTapParamId(tid, ID::time), desc + "Time", 0.0f,
                                                                100.0f, (tid + 1) * 5.0f));
        tap->addChild(std::make_unique<ssp::BaseFloatParameter>(getTapParamId(tid, ID::hpf), desc + "HPF", 0.0f,
                                                                20000.0f, 10.0f));
        tap->addChild(std::make_unique<ssp::BaseFloatParameter>(getTapParamId(tid, ID::lpf), desc + "LPF", 0.0f,
                                                                20000.0f, 20000.0f));
        tap->addChild(std::make_unique<ssp::BaseFloatParameter>(getTapParamId(tid, ID::noise), desc + "Dirt", 0.0f,
                                                                100.0f, 0.0f));
        tap->addChild(
            std::make_unique<ssp::BaseFloatParameter>(getTapParamId(tid, ID::pan), desc + "Pan", -1.0f, 1.0f, 0.0f));
        tap->addChild(std::make_unique<ssp::BaseFloatParameter>(getTapParamId(tid, ID::level), desc + "Level", 0.0f,
                                                                100.0f, 50.0f));
        tap->addChild(std::make_unique<ssp::BaseFloatParameter>(getTapParamId(tid, ID::feedback), desc + "Feedback", 0,
                                                                100.0f, 0.0f));
...
const String PluginProcessor::getInputBusName(int channelIndex) {
    static String inBusName[I_MAX] = { "In 1",     "In 2",     "Time 1 A", "Time 1 B", "Time 1 C",
                                       "Time 1 D", "Time 2 A", "Time 2 B", "Time 2 C", "Time 2 D" };
...
const String PluginProcessor::getOutputBusName(int channelIndex) {
    static String outBusName[O_MAX] = { "Out 1",   "Out 2",   "Tap 1 A", "Tap 1 B", "Tap 1 C",
                                        "Tap 1 D", "Tap 2 A", "Tap 2 B", "Tap 2 C", "Tap 2 D" };
